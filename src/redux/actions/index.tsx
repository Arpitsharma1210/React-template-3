...
export const apiCall = (
  ...
  const api = makeApiCall<any, any, any>(
    `APICALL_${endPoint}`, makeApiRequestObject<any, any>(endPoint, method),
  );

  return apiRequest(
    setRejectFunction(
      setResolveFunction(api, resolve), reject,
    ),
    data,
    isFormData,
  );
...
export const login = (formData: { email: string; password: string },
  resolve: any, reject: any) => action(LOGIN, { formData, resolve, reject });
