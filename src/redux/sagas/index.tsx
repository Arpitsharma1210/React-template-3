...
import { APICALL, ApiCall, FETCH_BASE_DATA, LOGIN, LOGOUT, PAGINATED_APICALL, PagedApiCall, action, fetchBaseData, logout, removeToken, updateToken } from "../actions";
import { HttpStatus, routes } from "../../utils";
import { apiCall, login, ping } from '../../api';
import { MetaData } from '../../models';
import { SagaIterator } from 'redux-saga';
import { AuthState, Role, defaultAuthState, getStateFromToken, getToken } from '../reducers/auth';
import { push } from 'connected-react-router';

declare type callback = (body: any) => any;
declare type errorHandler = (body: any, response: Response) => any;

export function* handleResponse(response: Response, callback: callback,
  error?: errorHandler, ignoreStatus = false): any {
  try {
    ...
    yield* handleResponse(
      ...
      function* (body) {
        ...
        if (newState?.role === Role.INVALID) {
          ...
          if (auth?.tempPass) {
            yield put(push(routes.setPassword));
          }
